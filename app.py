...
def parse_cell_coordinate(coord_string):
    """解析Excel单元格坐标，如 'A1' -> ('A', 1)"""
    try:
        # 使用正则表达式解析坐标
        ...
        for sheet_name in workbook.sheetnames:
            ...
            try:
                for comment_coord, comment_obj in worksheet.comments.items():
                    ...
                    try:
                        # 解析坐标
                        col_letter, row_num = parse_cell_coordinate(comment_coord)
...
if user_type == "管理员" and st.session_state.is_admin:
    ...
    if not st.session_state.logged_in:
        ...
        if matching_sheets:
            ...
            st.error(f"❌ 未找到门店 '{st.session_state.store_name}' 的报表"), coord_string.upper())
        if match:
