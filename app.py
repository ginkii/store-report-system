    ...
    try:
        ...
        if len(df_copy) >= 2:
            ...
            for i, col in enumerate(df_copy.columns):
                if i < len(second_row) and pd.notna(second_row.iloc[i]):
                    new_col_name = str(second_row.iloc[i]).strip()
                    if new_col_name and new_col_name.lower() not in ['nan', 'none', '']:
                        new_columns.append(new_col_name)
                    else:
                        # 如果第二行为空，保持原列名或生成默认列名
                        if i == 0:
                            new_columns.append("项目")
                        elif 'Unnamed' in str(col) and i == len(df_copy.columns) - 1:
            ...
            # 删除前两行（原表头和作为新表头的第二行）
            df_copy = df_copy.drop([df_copy.index[0], df_copy.index[1]]).reset_index(drop=True)
